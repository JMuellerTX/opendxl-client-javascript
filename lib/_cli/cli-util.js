...
module.exports = {
  ...
  /**
   * Returns whether certificate validation for the management server has been
   * switched off. The option is accepted both on the subcommand and on the
   * program, so both are consulted.
   * @param {Command} command - The Commander command to read the option from.
   * @returns {Boolean} `true` when validation should be skipped.
   */
  isInsecure: function (command) {
    return !!(command.insecure || (command.parent && command.parent.insecure))
  },
  /**
   * Checks the management service options which cannot be combined.
   * @param {Command} command - The Commander command to validate.
   * @returns {Error} An error describing the conflict, or `null` when the
   *   options are consistent.
   */
  validateManagementServiceOptions: function (command) {
    if (command.truststore && module.exports.isInsecure(command)) {
      return provisionUtil.createFormattedDxlError(
        'The -e (truststore) and --insecure options cannot be used together. ' +
        'Use -e to validate the management server against a private CA, or ' +
        '--insecure to skip validation entirely.')
    }
    return null
  },
  /**
  ...
  pullHostInfoFromCommand: function (hostname, command) {
    ...
    if (module.exports.isInsecure(command)) {
      hostInfo.insecure = true
      delete command.insecure
    }
    return hostInfo
  }
}
