 ...
 *   specified, the certificate is validated against the CAs which Node.js
 *   trusts by default.
 * @property {Boolean} [insecure=false] - If `true`, the management service's
 *   certificate is not validated at all. Mutually exclusive with
 *   `truststore`.
 * @private
 */
...
function ManagementService (hostInfo) {
  ...
  if (hostInfo.truststore && hostInfo.insecure) {
    throw new TypeError(
      'The truststore and insecure options are mutually exclusive')
  }
  this.trustStoreFile = hostInfo.truststore
  this.insecure = !!hostInfo.insecure
}
...
/**
 * Builds a hint describing how to resolve a TLS certificate error, to be
 * appended to the error message shown to the user.
 * @param {Error} error - The error raised by the HTTP request.
 * @returns {String} The hint, or an empty string when the error is unrelated
 *   to certificate validation.
 * @private
 */
function certificateErrorHint (error) {
  const code = error && error.code
  if (!code) {
    return ''
  }
  if (code === 'ERR_TLS_CERT_ALTNAME_INVALID') {
    return '. The server certificate is not valid for this host name. Use ' +
      'the host name from the certificate instead of an IP address, or ' +
      'pass --insecure to skip the check.'
  }
  if (code.indexOf('CERT') >= 0 || code.indexOf('SELF_SIGNED') >= 0 ||
      code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE') {
    return '. The management server certificate could not be validated. ' +
      'Export the CA which issued it and pass it with -e <ca_file>, or pass ' +
      '--insecure to disable validation.'
  }
  return ''
}
...
ManagementService.prototype.request = function (description, path, queryString,
  ...
  if (this.trustStoreFile) {
    ...
    console.warn('WARN: Certificate validation for the management server is ' +
      'disabled (--insecure). The connection is not protected against ' +
      'interception.')
    requestOptions.rejectUnauthorized = false
    requestOptions.requestCert = false
  } else {
    // Default: validate against the CAs which Node.js trusts, including the
    // host name. Use -e for a private CA or --insecure to opt out.
    requestOptions.rejectUnauthorized = true
    requestOptions.requestCert = true
  }
