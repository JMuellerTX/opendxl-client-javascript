...
function addSubscription (client, topic, messageType,
  ...
  if (subscribeToTopic && topic) {
    ...
    if (!topicMessageTypes) {
      if (client._mqttClient && !client._mqttClient.disconnecting) {
...
function removeSubscription (client, topic, messageType, callback) {
  ...
  if (topic) {
    ...
    if (topicMessageTypes) {
      ...
      if (client._mqttClient && !client._mqttClient.disconnecting &&
        !subscriptionsByMessageType[topic]) {
...
Client.prototype.connect = function (callback) {
  ...
  mqttClient.on('error', function (error) {
    // mqtt.js 5.x surfaces socket errors (for example ECONNRESET when the
    // broker closes the connection after a DISCONNECT) through the error
    // event. While the client is shutting down these are expected, so only
    // log them at debug level instead of reporting them to the application.
    if (mqttClient.disconnecting) {
      debug('Ignoring MQTT client error while disconnecting', error)
      return
    }
...
Client.prototype.destroy = function (callback) {
  ...
  if (this._mqttClient && !this._mqttClient.disconnecting) {
